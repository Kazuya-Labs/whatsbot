...
- Reconnection is self-managed: `src/connection/update.js` re-calls `start()` up to 3× (5s delay; the restart fn is injected as a param to avoid circular imports). On `loggedOut` it stops; delete `auth_info_baileys/` and restart to re-pair.
...
- Owner-only dev backdoor: any message starting with `>` is `eval()`'d (`src/connection/message.js`).
